...
func (r *reflector) listAndWatch(stopCh <-chan struct{}) (error, []client.ResponseError) {
	...
	if err != nil {
		return fmt.Errorf("unable save objects: %s", err), nil
	}
	klog.Infof("replace store objects of type %s with: ", r.expectType.TypeName())
	klog.Infoln(r.store.List()...)

	return r.watchHandler(respCh, stopCh)
}

// watchHandler watches respChan and keep store with latest objects.
func (r *reflector) watchHandler(respCh <-chan client.Response, stopCh <-chan struct{}) (error, []client.ResponseError) {
	var err error
...
func (r *reflector) eventHandler(raw json.RawMessage) error {
	...
	klog.Infof("get %s event of type %s: %v", event.Mutation, r.expectType.TypeName(), obj)
...
func (r *reflector) watchErrorHandler(err error, respErrs []client.ResponseError) {
	switch {
...
// todo: support resync, see more: https://pkg.go.dev/k8s.io/client-go/tools/cache#DeltaFIFO.Resync
