...
var _ = Describe("SecurityPolicy", func() {
	...
	Context("environment with endpoints provide public http service [Feature:TCP] [Feature:ICMP]", func() {
		...
		AfterEach(func() {
			Expect(e2eEnv.EndpointManager().CleanMany(ctx, nginx, server01, server02, db01, db02, client)).Should(Succeed())
			Expect(e2eEnv.CleanObjects(ctx, nginxGroup, serverGroup, dbGroup)).Should(Succeed())
		})
		...
		When("limits tcp packets between components", func() {
			...
			AfterEach(func() {
				Expect(e2eEnv.CleanObjects(ctx, nginxPolicy, serverPolicy, dbPolicy)).Should(Succeed())
			})
			...
			When("add endpoint into the database group", func() {
				...
				AfterEach(func() {
					Expect(e2eEnv.EndpointManager().Delete(ctx, db03.Name)).Should(Succeed())
				})
		...
		When("limits icmp packets between components", func() {
			...
			AfterEach(func() {
				Expect(e2eEnv.CleanObjects(ctx, icmpAllowPolicy, icmpDropPolicy)).Should(Succeed())
			})
	...
	Context("endpoint isolation [Feature:ISOLATION]", func() {
		...
		AfterEach(func() {
			Expect(e2eEnv.CleanObjects(ctx, forensicGroup)).Should(Succeed())
			Expect(e2eEnv.EndpointManager().CleanMany(ctx, ep01, ep02, ep03, ep04)).Should(Succeed())
		})
		...
		When("Isolate endpoint", func() {
			...
			AfterEach(func() {
				Expect(e2eEnv.CleanObjects(ctx, isolationPolicy)).Should(Succeed())
			})
			...
			It("Isolated endpoint should not allow to communicate with all of endpoint", func() {
				...
				assertReachable([]*model.Endpoint{ep02, ep03, ep04}, []*model.Endpoint{ep01}, "TCP", false)
			})
		})
		...
		When("Forensic endpoint", func() {
			...
			BeforeEach(func() {
				...
				addIngressRule(forensicPolicy, "TCP", 3306, forensicGroup.Name)
			...
			AfterEach(func() {
				Expect(e2eEnv.CleanObjects(ctx, forensicPolicy)).Should(Succeed())
			})
			...
			It("Isolated endpoint should not allow to communicate with all of endpoint except forensic defined allowed endpoint", func() {
				...
				assertReachable([]*model.Endpoint{ep03, ep04}, []*model.Endpoint{ep01}, "TCP", false)
				assertReachable([]*model.Endpoint{ep01}, []*model.Endpoint{ep02, ep03, ep04}, "TCP", false)

				assertReachable([]*model.Endpoint{ep02}, []*model.Endpoint{ep01}, "TCP", true)
			})
	...
	Context("environment with endpoints provide internal udp service [Feature:UDP] [Feature:IPBlocks]", func() {
		...
		BeforeEach(func() {
			...
			ntpProduction = newGroup("ntp-production", map[string]string{"component": "ntp", "env": "production"})
			ntpDevelopment = newGroup("ntp-development", map[string]string{"component": "ntp", "env": "development"})

			Expect(e2eEnv.EndpointManager().SetupMany(ctx, ntp01, ntp02, client01, client02)).Should(Succeed())
			Expect(e2eEnv.SetupObjects(ctx, ntpProduction, ntpDevelopment)).Should(Succeed())
		})

		AfterEach(func() {
			Expect(e2eEnv.EndpointManager().CleanMany(ctx, ntp01, ntp02, client01, client02)).Should(Succeed())
			Expect(e2eEnv.CleanObjects(ctx, ntpProduction, ntpDevelopment)).Should(Succeed())
		})
		...
		When("limits udp packets by ipBlocks between server and client", func() {
			...
			BeforeEach(func() {
				ntpProductionPolicy = newPolicy("ntp-production-policy", tier1, 50, []string{ntpProduction.Name}, nil)
				addIngressRule(ntpProductionPolicy, "UDP", ntpPort, productionCidr)

				ntpDevelopmentPolicy = newPolicy("ntp-development-policy", tier1, 50, []string{ntpDevelopment.Name}, nil)
			...
			AfterEach(func() {
				Expect(e2eEnv.CleanObjects(ctx, ntpProductionPolicy, ntpDevelopmentPolicy)).Should(Succeed())
			})
			...
			It("should allow normal packets and limits illegal packets", func() {
				...
				assertFlowMatches(&SecurityModel{
					...
					Groups:    []*groupv1alpha1.EndpointGroup{ntpProduction, ntpDevelopment},
