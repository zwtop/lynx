...
const (
	startNewVM = `
		...
		if [[ ${tcp_port} != 0 ]]; then
			nohup ip netns exec ${netns} nc -kl ${tcp_port}  &>/dev/null &
		fi

		if [[ ${udp_port} != 0 ]]; then
			nohup ip netns exec ${netns} nc -kul ${udp_port} &>/dev/null &
		fi

		sleep 0.5s # wait for nohup background run
	`
	...
	udpReachable = `
		...
		ip netns exec ${local_netns} nc -zuv -w ${timeout} ${remote_ipaddr} ${remote_port}
	`
